...
function renderCompress(){
  ...
  if(!s.file) return dropZone("compress","Arrastra un PDF aquí o haz clic para seleccionar","Solo archivos PDF · Máximo 50 MB");
...
async function doCompress(){
  ...
  }catch(e){console.error(e);showToast("Error al comprimir","error")}
...
function renderSplit(){
  const s=state.split;
  if(!s.file) return dropZone("split","Arrastra un PDF aquí o haz clic para seleccionar","Se mostrarán opciones de división");
...
async function doMerge(){
  ...
  try{
    ...
    const u8=await merged.save({useObjectStreams:true});
...
function renderOcr(){
  const s=state.ocr;
  if(!s.file) return dropZone("ocr","Arrastra un PDF escaneado aquí","El texto será extraído en el servidor mediante IA");
