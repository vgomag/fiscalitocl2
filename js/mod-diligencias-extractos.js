    ...
    html+=`<tr class="dil-row" style="border-bottom:1px solid var(--border);transition:background .1s" onmouseover="this.style.background='var(--surface2)'" onmouseout="this.style.background=''">
      ...
      <td style="padding:8px 10px;text-align:center;white-space:nowrap">
        <div style="display:flex;gap:3px;justify-content:center">
          ...
          ${!d.is_processed&&/expediente|completo|foliado/i.test(d.file_name)?`<button class="btn-action" title="Analizar expediente — identificar diligencias" onclick="analyzeExpediente('${d.id}')" style="color:var(--gold);font-weight:bold">🔍</button>`:''}
...
/* ══════════════════════════════════════════════════════════════
   ANALIZAR EXPEDIENTE COMPLETO — Identifica diligencias con IA
   ══════════════════════════════════════════════════════════════ */
async function analyzeExpediente(dilId){
  ...
  await sb.from('diligencias').update({processing_status:'processing'}).eq('id',dilId);
  showToast('🔍 Analizando expediente con IA — identificando diligencias…');
  ...
  try{
    ...
    const data=await res.json();
    if(!res.ok||!data.ok) throw new Error(data.error||'Error en análisis');

    /* Update parent record with full text */
    await sb.from('diligencias').update({
      extracted_text:data.extractedText,
      ai_summary:data.aiSummary||null,
      is_processed:true,
    ...
    /* Create individual diligencia records for each identified one */
    const dils=data.diligencias||[];
    if(dils.length>0){
      const existing=await sb.from('diligencias').select('diligencia_label')
        .eq('case_id',currentCase.id);
      const existingLabels=new Set((existing.data||[]).map(d=>d.diligencia_label));
      ...
      if(inserts.length>0){
        const{error:insertErr}=await sb.from('diligencias').insert(inserts);
        if(insertErr)console.warn('Insert diligencias:',insertErr.message);
      }

      showToast(`✅ ${dils.length} diligencia(s) identificada(s) en "${dil.file_name}"`);
    } else {
      showToast('✅ Expediente procesado (sin diligencias individuales identificadas)');
    }
