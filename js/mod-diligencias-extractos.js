...
async function processDiligenciaOCR(dilId){
  ...
  showToast('⏳ Procesando: '+dil.file_name);

  try{
    let extractedText='';

    /* Try reading from Drive if we have a drive_file_id */
    if(dil.drive_file_id){
      try{
        const res=await fetch('/.netlify/functions/drive',{
          method:'POST',
          headers:{'Content-Type':'application/json'},
          body:JSON.stringify({action:'read',fileId:dil.drive_file_id})
        });
        if(res.ok){
      ...
      }catch(e){console.warn('Drive read failed:',e);}
    }

    /* If no text from Drive, use AI to analyze based on filename and context */
    if(!extractedText){
      const prompt=`Analiza el siguiente documento del expediente disciplinario "${currentCase.name}".
Nombre del archivo: ${dil.file_name}
Tipo clasificado: ${dil.diligencia_type}
Ruta: ${dil.file_path||dil.file_name}

Como no puedo acceder directamente al contenido binario del archivo, genera una nota indicando que el documento requiere revisión manual para extracción de texto.
Describe qué tipo de contenido se esperaría encontrar en un documento con este nombre y clasificación en el contexto de un procedimiento disciplinario chileno.`;
      ...
      const r=await authFetch(CHAT_ENDPOINT,{
        ...
        body:JSON.stringify({
          ...
          system:'Eres un asistente jurídico experto en procedimientos disciplinarios chilenos. Analiza documentos del expediente.',
          messages:[{role:'user',content:prompt}]
        })
      });
      const data=await r.json();
      extractedText=(data.content||[]).filter(b=>b.type==='text').map(b=>b.text).join('')||'';
    }
    ...
    if(extractedText.length>50){
      try{
        const r2=await authFetch(CHAT_ENDPOINT,{
          method:'POST',
          headers:{'Content-Type':'application/json'},
          body:JSON.stringify({
            model:'claude-sonnet-4-20250514',max_tokens:300,
            system:'Resume documentos jurídicos en máximo 3 oraciones concisas. Solo el resumen, sin preámbulos.',
            messages:[{role:'user',content:`Resume este documento tipo "${dil.diligencia_type}" del expediente:\n\n${extractedText.substring(0,4000)}`}]
          })
        });
        const d2=await r2.json();
        aiSummary=(d2.content||[]).filter(b=>b.type==='text').map(b=>b.text).join('')||'';
    ...
    showToast('✅ Procesado: '+dil.file_name);
    await loadDiligenciasTab();
