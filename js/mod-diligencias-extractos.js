...
/* ── Procesar OCR de una diligencia individual ── */
async function processDiligenciaOCR(dilId){
  ...
  showToast('⏳ Procesando con IA (servidor): '+dil.file_name);

  try{
    /* Call server-side OCR function — downloads PDF + extracts text + generates summary */
    const res=await fetch('/.netlify/functions/ocr',{
      method:'POST',
      headers:{'Content-Type':'application/json','x-auth-token':session?.access_token||''},
      body:JSON.stringify({
        fileId:dil.drive_file_id,
        fileName:dil.file_name,
        diligenciaType:dil.diligencia_type
      })
    });
    const data=await res.json();
    if(!res.ok||!data.ok) throw new Error(data.error||'Error en OCR servidor');
    ...
    showToast(`✅ Procesado: ${dil.file_name} (${data.charCount} caracteres)`);
    await loadDiligenciasTab();
...
/* ══════════════════════════════════════════════════════════════
   ANALIZAR EXPEDIENTE — Pipeline de 2 etapas para archivos grandes
   Etapa 1: OCR (extraer texto) — una llamada
   Etapa 2: Análisis (identificar diligencias) — llamada separada
   Cada etapa cabe dentro del timeout de 10s de Netlify
   ══════════════════════════════════════════════════════════════ */
async function analyzeExpediente(dilId){
  ...
  let extractedText=dil.extracted_text||'';

  /* ── ETAPA 1: Extraer texto (si no se ha hecho) ── */
  if(!extractedText||extractedText.length<100){
    if(!dil.drive_file_id){showToast('⚠️ Sin archivo Drive vinculado');return;}

    await sb.from('diligencias').update({processing_status:'processing'}).eq('id',dilId);
    showToast('📥 Etapa 1/2: Extrayendo texto del expediente…');
    ...
    try{
      ...
      const data=await res.json();
      if(!res.ok||!data.ok) throw new Error(data.error||'Error extrayendo texto');
      ...
      showToast(`✅ Texto extraído (${data.charCount} caracteres)`);
    }catch(err){
      await sb.from('diligencias').update({processing_status:'error'}).eq('id',dilId);
      showToast('❌ Error extrayendo: '+err.message);
      console.error('analyzeExpediente etapa 1:',err);
      return;
    }
  }

  if(!extractedText||extractedText.length<100){
    showToast('⚠️ Texto extraído insuficiente para analizar');
    return;
  }

  /* ── ETAPA 2: Identificar diligencias con IA ── */
  showToast('🔍 Etapa 2/2: Identificando diligencias con IA…');
  ...
  try{
    ...
    const data=await res.json();
    if(!res.ok||!data.ok) throw new Error(data.error||'Error analizando');

    const dils=data.diligencias||[];
    if(dils.length>0){
      /* Avoid duplicates */
      const{data:existing}=await sb.from('diligencias').select('diligencia_label')
        .eq('case_id',currentCase.id);
      const existingLabels=new Set((existing||[]).map(d=>d.diligencia_label));

      const inserts=[];
      dils.forEach((d,i)=>{
        const label=d.titulo||d.tipo||'Diligencia '+(i+1);
        ...
        inserts.push({
          ...
          case_id:currentCase.id,
          user_id:session.user.id,
          diligencia_type:d.tipo||'otro',
          diligencia_label:label,
          file_name:dil.file_name+(d.fojas?' [pp.'+d.fojas+']':''),
          ...
          fojas_inicio:d.fojas?parseInt(d.fojas.split('-')[0])||null:null,
          fojas_fin:d.fojas?parseInt(d.fojas.split('-')[1])||null:null,
          ai_summary:d.resumen||null,
          extracted_text:d.texto_inicio||null,
          is_processed:true,
          processing_status:'completed',
          order_index:i+1,
        });
      ...
      if(inserts.length>0){
        ...
        if(ie)console.warn('Insert diligencias:',ie.message);
      }

      showToast(`✅ ${dils.length} diligencia(s) identificada(s) en "${dil.file_name}"`);
    } else {
      showToast('✅ Procesado (no se identificaron diligencias individuales)');
    }

    await loadDiligenciasTab();

  }catch(err){
    showToast('❌ Error analizando: '+err.message);
    console.error('analyzeExpediente etapa 2:',err);
  }
}
