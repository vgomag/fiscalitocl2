...
async function transcribeAudio(){
  ...
  for(let attempt=0;attempt<=T_MAX_RETRIES;attempt++){
    try{
      ...
      setProgress(15,'Codificando audio en base64…');
      ...
      if(data?.text){
        ...
        showToast('✓ Transcripción completa'+(sc>1?' · '+sc+' hablantes':''));
...
La transcripción contiene expresiones propias del lenguaje oral. Conserva la redacción en primera persona y el estilo expresivo del declarante, realizando únicamente correcciones gramaticales menores.

INSTRUCCIONES:
- Mejorar gramática, claridad y coherencia del texto
- Eliminar muletillas ("eh", "mmm", "o sea") y repeticiones innecesarias
- Conservar la estructura lógica y la secuencia cronológica
- Respetar terminología jurídica y nombres propios exactamente como aparecen
- No agregar información nueva ni interpretar intenciones
- Usar tono formal, claro y preciso

FORMATO: Texto corregido en formato pregunta-respuesta, con párrafos separados. Sin comentarios ni marcas de edición.`;
...
FORMATO: Acta formal con encabezado institucional, cuerpo de declaración en párrafos y cierre.`;

const T_PROMPT_EXPEDIENTE = `Eres Fiscalito, asistente jurídico de la Universidad de Magallanes.

INSTRUCCIÓN: Estructura la transcripción como ACTA FORMAL institucional completa. DEBES incluir en el encabezado TODOS los datos del expediente que se proporcionan abajo. El acta debe tener:
...
2. CUERPO:
   - Presentar la declaración estructurada con las correcciones gramaticales necesarias
   - Conservar la redacción en primera persona
   - Mantener nombres propios, fechas y cifras exactos

3. CIERRE FORMAL:
   - Fórmula de cierre: "Leída que le fue su declaración, se ratifica y firma"
   - Espacios para firmas del/la declarante, fiscal y actuario/a

REGLAS:
- Eliminar muletillas y repeticiones sin alterar el sentido
- No agregar información nueva
- Tono formal, claro y preciso coherente con un documento legal
...
- ROL: ${caseObj.rol||'[ROL]'}
- Tipo de procedimiento: ${caseObj.tipo_procedimiento||'[TIPO PROCEDIMIENTO]'}
- Materia investigada: ${caseObj.materia||'[MATERIA]'}
- Protocolo: ${caseObj.protocolo||'[PROTOCOLO]'}
- Resolución que instruye: ${caseObj.nueva_resolucion||'[RESOLUCIÓN]'}
- Fecha resolución: ${caseObj.fecha_resolucion||'[FECHA]'}
- Denunciante(s): ${_fmtArr(caseObj.denunciantes)||'[DENUNCIANTE]'}
- Estamento denunciante: ${_fmtArr(caseObj.estamentos_denunciante)||'[ESTAMENTO]'}
- Denunciado/a(s): ${_fmtArr(caseObj.denunciados)||'[DENUNCIADO/A]'}
- Estamento denunciado/a: ${_fmtArr(caseObj.estamentos_denunciado)||'[ESTAMENTO]'}`;

  // Cargar solo participantes (fiscal y actuario para encabezado del acta)
  try{
    const{data:parts}=await sb.from('case_participants')
      .select('role,name,estamento,rut')
      .eq('case_id',cid)
      .in('role',['Fiscal','fiscal','Actuario','actuario','Actuaria','actuaria','Investigador','investigador','Investigadora','investigadora'])
      .limit(5);
    if(parts?.length){
      parts.forEach(p=>{
        if((p.role||'').toLowerCase().includes('fiscal')||( p.role||'').toLowerCase().includes('investigad'))
          ctx+='\n- FISCAL/INVESTIGADOR(A): '+p.name+(p.rut?' RUT: '+p.rut:'');
        if((p.role||'').toLowerCase().includes('actuari'))
          ctx+='\n- ACTUARIO/A: '+p.name;
      });
    }
  }catch(e){console.warn('loadFullCaseContext:',e);}

  return ctx;
}

/* ════════════════════════════════════════════════════════
   ESTRUCTURAR — con reintentos y progreso visual
   ════════════════════════════════════════════════════════ */
async function structureTranscripcion(){
  ...
  for(let attempt=0;attempt<=T_MAX_RETRIES;attempt++){
    try{
      ...
      setProgress(10,'Preparando contexto…');
      let caseCtx='';
      if(transcripcion.selectedMode==='con_expediente'&&lnk){
        setProgress(15,'Cargando datos del expediente…');
        caseCtx=await loadFullCaseContext(lnk);
        caseCtx+='\n\nFecha de la declaración: '+fecha;
      } else if(lnk){
        caseCtx='\n\nDATOS DEL EXPEDIENTE:\n- Expediente: '+(lnk.name||'[EXPEDIENTE]')+'\n- ROL: '+(lnk.rol||'[ROL]')+'\n- Fecha declaración: '+fecha;
      } else {
        caseCtx='\n\nFecha de declaración: '+fecha;
      }

      const docCtx=transcripcion.baseDocText
        ?'\n\nDOCUMENTO BASE DE REFERENCIA (acta previa):\n'+transcripcion.baseDocText.substring(0,2000)
        :'';

      const systemPrompt=basePrompt+caseCtx+docCtx;

      // ── Dividir en partes si es largo ──
      setProgress(25,'Estructurando declaración…');
      const MAX_CHUNK=3000;
      const raw=transcripcion.rawText;

      if(raw.length<=MAX_CHUNK){
        setProgress(40,'Enviando a IA…');
        ...
        // Transcripción larga: por partes
        const parts=[];
        for(let i=0;i<raw.length;i+=MAX_CHUNK){parts.push(raw.substring(i,i+MAX_CHUNK));}
        const messages=[];

        for(let i=0;i<parts.length;i++){
          const isLast=i===parts.length-1;
          const partLabel='PARTE '+(i+1)+' DE '+parts.length;
          const pctBase=25+Math.round((i/parts.length)*60);
          setProgress(pctBase,'Procesando '+partLabel+'…');

          if(i===0){
            messages.push({role:'user',content:'La transcripción se entregará en '+parts.length+' partes. Incorpora cada parte.\n\n'+partLabel+':\n\n'+parts[i]+(isLast?'\n\nEsta es la última parte. Genera ahora el documento completo estructurado.':'\n\nAcusa recibo y espera la siguiente parte.')});
          } else {
            messages.push({role:'assistant',content:'Recibida la parte '+i+' de '+parts.length+'. Continúe.'});
            messages.push({role:'user',content:partLabel+':\n\n'+parts[i]+(isLast?'\n\nÚltima parte. Genera el Texto Refundido completo con TODAS las partes.':'\n\nAcusa recibo.')});
          }

          if(isLast){
            setProgress(85,'Generando acta final…');
            const body={model:'claude-sonnet-4-20250514',max_tokens:4000,system:systemPrompt,messages};
            const resp=await authFetch(ep,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});
